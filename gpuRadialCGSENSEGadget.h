...
namespace Gadgetron{
  ...
  class EXPORTGADGETS_CPURADIAL gpuRadialCGSENSEGadget :
    public Gadget2< ISMRMRD::AcquisitionHeader, hoNDArray< std::complex<float> > >
  {

  public:

    gpuRadialCGSENSEGadget();
    virtual ~gpuRadialCGSENSEGadget();
  ...
  protected:
    ...
    float oversampling_factor_ = 1.25;
  
    std::vector<float> dcf;
    
    bool cal_radial2D_dcf ( std::vector<float>& dcf, size_t dcf_samples_per_profile_, size_t dcf_profiles_per_frame_, double fov1, double fov2);
    Trajectory2D traj;
